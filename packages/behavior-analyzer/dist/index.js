...
export class BehaviorAnalyzer {
    ...
    analyze() {
        ...
        const overallHumanScore = (movementSmoothnessScore * 0.35 +
    ...
    calculateMovementSmoothness() {
        ...
        for (let i = 2; i < mouseEvents.length; i++) {
            ...
            if (!e1.x || !e1.y || !e2.x || !e2.y || !e3.x || !e3.y)
                continue;
    ...
    calculateReactionTime() {
        ...
        if (keyEvents.length + mouseEvents.length < 5)
            return 0.5; // Not enough data
        ...
        if (avgReactionTime < 50)
            return 0.1; // Too fast, likely a bot
        if (avgReactionTime > 1000)
            return 0.3; // Too slow, might be a bot
