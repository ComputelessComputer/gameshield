...
export class PuzzleGame extends BaseGame {
  ...
  /** Puzzle pieces */
  private pieces: PIXI.Sprite[] = [];
  
  /** Empty cell position */
  private emptyCell: { row: number, col: number } = { row: 0, col: 0 };
  
  /** Original image */
  private image: PIXI.Sprite | null = null;
  ...
  private solvedPositions: { x: number, y: number }[] = [];
  
  /** Custom configuration */
  private puzzleConfig: Required<PuzzleGameConfig>;
  
  /** Piece metadata */
  private pieceMetadata: Map<PIXI.Sprite, { row: number, col: number, originalIndex: number }> = new Map();
   ...
   * @private
   */
  private getEmptyCellNeighbors(): PIXI.Sprite[] {
    const neighbors: PIXI.Sprite[] = [];
   ...
   * @private
   */
  private findPieceAt(row: number, col: number): PIXI.Sprite | null {
    const pieceWidth = this.config.width / this.columns;
    const pieceHeight = this.config.height / this.rows;
   ...
   * @private
   */
  private handlePieceClick(piece: PIXI.Sprite): void {
   ...
   * @private
   */
  private swapWithEmptyCell(piece: PIXI.Sprite): void {
   ...
   * @private
   */
  private checkPuzzleSolved(): boolean {
   ...
   * @private
   */
  private getDifficultyBasedShuffleMoves(): number {
