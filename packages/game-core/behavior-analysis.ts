...
export interface BehaviorMetrics {
  // Movement patterns
  movementSmoothness: number;  // Higher values indicate smoother, more human-like movement
  movementVariability: number; // Variability in speed and direction
  
  // Timing metrics
  reactionTimes: number[];     // Time to react to game events
  averageReactionTime: number;
  
  // Decision making
  decisionConsistency: number; // Consistency in similar situations
  errorRate: number;           // Rate of mistakes/corrections
  
  // Interaction patterns
  interactionDensity: number;  // Number of interactions per second
  pausePatterns: number[];     // Durations of pauses between actions
...
export class BehaviorAnalyzer {
  ...
  private expectedReactionEvents: Map<string, number> = new Map();
   ...
   */
  private calculateMovementVariability(): number {
    if (this.interactionPoints.length < 5) {
      return 0.5; // Not enough data
    }
    
    const movePoints = this.interactionPoints.filter(p => p.type === 'move');
    if (movePoints.length < 5) return 0.5;
   ...
   */
  isHumanBehavior(): { isHuman: boolean; confidence: number; } {
