 ...
 */
export class LocalStorageAnalyticsProvider {
    constructor() {
        this.VERIFICATION_KEY = 'gameshield_verification_data';
        this.MALICIOUS_KEY = 'gameshield_malicious_data';
    ...
    async getStats(filters) {
        ...
        try {
            const verificationData = JSON.parse(localStorage.getItem(this.VERIFICATION_KEY) || '[]');
            const maliciousData = JSON.parse(localStorage.getItem(this.MALICIOUS_KEY) || '[]');
            // Apply filters
    ...
    async getVerificationData(filters) {
        if (typeof window === 'undefined')
            return [];
        try {
            const data = JSON.parse(localStorage.getItem(this.VERIFICATION_KEY) || '[]');
    ...
    async getMaliciousActivityData(filters) {
        if (typeof window === 'undefined')
            return [];
        try {
            const data = JSON.parse(localStorage.getItem(this.MALICIOUS_KEY) || '[]');
            return this.applyFiltersToData(data, filters);
        }
        catch (error) {
